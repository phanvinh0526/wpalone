...
function tbbs_RenderFieldTemplate( $field )
{
	...
	switch ( $type ) {
		...
		case 'imagesize': 
			$_description = isset( $description ) ? "<p>{$description}</p>" : '';
			$_options = "";
			$_value = isset( $value ) ? $value : '';
			$imgsize_arr = get_intermediate_image_sizes();

			if( count( $imgsize_arr ) > 0 ) :
				foreach( $imgsize_arr as $imgsize_item ) :
					$selected = ( $_value == $imgsize_item ) ? 'selected' : '';
					$_options .= sprintf( '<option value="%s" %s>%s</option>', $imgsize_item , $selected, $imgsize_item );
				endforeach;
			endif;
...
function tbbs_register_scripts()
{
	...
	foreach( $scripts as $index => $script ) :
		...
		$inc = 0;
		if( isset( $manager_scripts[$script['handle'].'_'.$script['type']] ) ) :
			$inc = (int) $manager_scripts[$script['handle'].'_'.$script['type']];
		else :
			if( isset( $script['include'] ) ) :
				$inc = (int) $script['include'];
			endif;
		endif;
