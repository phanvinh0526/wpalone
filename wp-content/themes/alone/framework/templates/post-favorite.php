<?php
class bearstheme_NectarLove {
	public $post_id;
	 function __construct($post_id = null)   {
		$this->post_id = $post_id;
        add_action('wp_enqueue_scripts', array($this, 'enqueue_scripts'));
        add_action('wp_ajax_nectar-love', array($this, 'bearstheme_ajax_love'));
		add_action('wp_ajax_nopriv_nectar-love', array($this, 'bearstheme_ajax_love'));
	}
	...
	function bearstheme_ajax_love($post_id) {
		//update
		if( isset($_POST['loves_id']) ) {
			$post_id = str_replace('nectar-love-', '', $_POST['loves_id']);
			echo $this->bearstheme_love_post($post_id, 'update');
		}
		//get
		else {
			$post_id = str_replace('nectar-love-', '', $_POST['loves_id']);
			echo $this->bearstheme_love_post($post_id, 'get');
		}
		exit;
	}

	function bearstheme_love_post($post_id, $action = 'get')
	{
	...
	function add_love() {
		...
		$output = $this->bearstheme_love_post($post->ID);
...
$GLOBALS['bearstheme_post_favorite'] = new bearstheme_NectarLove();
function bearstheme_post_favorite() {
	global $bearstheme_post_favorite;
	echo $bearstheme_post_favorite->add_love();
}
?>
