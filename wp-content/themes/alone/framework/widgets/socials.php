<?php
class Bearstheme_Social_Widget extends WP_Widget {
    function __construct() {
        parent::__construct(
                'social_widget', // Base ID
                __('Socials', 'bearsthemes'), // Name
                array('description' => __('Social Widget', 'bearsthemes'),) // Args
        );
    }
    function widget($args, $instance) {
        ...
        <div class='social-wrap'>
            ...
            for ($i = 1; $i <= 6; $i++) {
                if($icon_social[$i]):
                ?>
				<a class="<?php echo esc_attr(str_replace('fa fa-', 'bt-', $icon_social[$i])); ?>" href="<?php echo esc_url($link_social[$i]); ?>">
    ...
    function form($instance) {
		...
		?>
		<p>
            <label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php _e('Title:', 'bearsthemes'); ?></label>
        ...
        for ($i = 1; $i <= 6; $i++) {
            ?>
            <p>
                <label for="<?php echo esc_url($this->get_field_id('icon_social_' . $i)); ?>"><?php _e('Social Icon:', 'bearsthemes');
            echo $i; ?></label>
            ...
            <p>
                <label for="<?php echo esc_url($this->get_field_id('link_social_' . $i)); ?>"><?php _e('Social Link:', 'bearsthemes');
            echo $i; ?></label>
        ...
        <?php } ?>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('el_class')); ?>"><?php _e('Extra Class:', 'bearsthemes'); ?></label>
...
/**
 * Class Bearstheme_Social_Widget
 */
function bearstheme_register_social_widget() {
    register_widget('Bearstheme_Social_Widget');
}
add_action('widgets_init', 'bearstheme_register_social_widget');
?>
